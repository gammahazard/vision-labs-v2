...
The first time you open the dashboard on a fresh install, after the forced admin password rotation, you'll be redirected to `/setup.html`. The wizard walks through:

1. **Hardware detection** — orchestrator spawns a one-shot `nvidia-smi` probe, shows your GPUs + recommends a hardware tier (small / mid / full) + estimates how many cameras your VRAM can support. Click **Apply this configuration** to write the chosen GPU placement + model choices to `.env` and recreate the affected services automatically.
2. **Add first camera** — Scan your network for ONVIF-compatible cameras, OR enter an RTSP URL manually. Built-in `ffprobe` Test Connection button. Camera ID is auto-assigned to the next free slot (cam1 first). Skip with one click to add cameras later from the Cameras tab.
...
Telegram-bot setup is not in the v1 wizard — configure it via `.env` and the in-dashboard Telegram Access Manager.
...
|----------|:--------:|-------------|
| `CAMERA_IP` | Optional | IP address of the primary camera (legacy; the registry now holds per-camera URLs) |
| `CAMERA_USER` | Optional | Camera login username (legacy) |
| `CAMERA_PASSWORD` | Optional | Camera login password (legacy) |
| `CAM1_RTSP_URL` | Optional | RTSP sub-stream URL for cam1 (override registry). cam2-5 use the registry exclusively. |
| `CAM1_RTSP_MAIN` | Optional | RTSP main-stream URL for cam1 |
