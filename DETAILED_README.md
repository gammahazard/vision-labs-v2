...
| **grafana** | — | Monitoring dashboards, linked from the system monitor page. Bound to `127.0.0.1` (off the LAN); anonymous-Viewer + admin password from `GRAFANA_ADMIN_PASSWORD` |
...
| `TARGET_FPS` | No | Ingester sub-stream target FPS — `.env.example` ships with `TARGET_FPS=10`; if unset the docker-compose fallback is 15. **Redis config `target_fps` overrides this dynamically.** |
...
| `GRAFANA_ADMIN_PASSWORD` | No | Grafana admin login password. Auto-generated by `scripts/install-linux.sh`. Never hardcode it (the repo is public). The embedded-Viewer panels don't depend on it. |
| `METRICS_TOKEN` | No | Bearer token the dashboard's `/metrics` requires. Auto-generated by `scripts/install-linux.sh` (written to `.env` + the gitignored `services/prometheus/metrics_token` Prometheus reads). Empty = `/metrics` served unauthenticated with a logged warning. |
| `ENABLE_LOCATE` / `LOCATE_API_URL` / `LOCATE_GPU` | No | Set by the opt-in `docker-compose.locate.yml` overlay only. Enable the AI-tab "Locate" tool; `LOCATE_GPU` pins the GPU for the grounding model. Not used by the default stack. |
...
- **Vision tab**: upload images or capture live frames for standalone MiniCPM-V analysis. When the opt-in locate service is running (`docker-compose.locate.yml`), a 📍 **Locate** button also appears — type what to find and get the image back with boxes drawn + a download link.
...
People count, inference time, GPU status, Redis memory cards, plus an "Open Grafana ↗" link (Grafana is bound to `127.0.0.1` — open it on the host or via `ssh -L 3000:localhost:3000 <host>`; it's no longer embedded in an iframe).
...
**With QNAP** (`docker-compose.qnap.yml` overlay): the 5 `qnap-*` volumes (`qnap-snapshots`, `qnap-events`, `qnap-telegram`, `qnap-videos`, `qnap-clips`) flip to CIFS mounts pointing at `//QNAP_IP/vision-labs/<subdir>`. The recordings bind mount can stay local or be swapped to an NFS mount — code paths inside containers (`/recordings/...`) stay identical either way.
...
- **Only the dashboard (`:8080`, auth-gated) is LAN-exposed.** Grafana (`:3000`), Prometheus (`:9090`), and the metrics exporters are bound to `127.0.0.1` (off the LAN). Portainer (`:9443`) is also loopback-bound. The dashboard's `/metrics` rides on `:8080` so it's gated by a bearer token (`METRICS_TOKEN`); Prometheus presents it via a `credentials_file`. Reach the host-only UIs via SSH tunnel.
