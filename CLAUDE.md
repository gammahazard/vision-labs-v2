...
- 365 tests in `/tests/`. Run via `source .venv-test/bin/activate && pytest -q`.
...
| **`tests.yml` (pytest)** | Runs the 365-test pytest suite on every push to main + every PR. Required check before PR merge. Includes 53 orchestrator tests covering the profile allowlist gate, config-apply allowlist, cred scrubbing on the audit stream, and the `desired_profiles → None`-on-Redis-error sentinel (regression here would tear down every camera on a transient Redis hiccup). | `.github/workflows/tests.yml` |
...
1. Run the test suite. 365 should pass.
