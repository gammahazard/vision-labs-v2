...
- **Training pipeline** at `scripts/vehicle_attributes/` — one-time offline GPU runs that produce `color_head.pth` (frozen-backbone, VeRi-776, ~20 min on 3090) and `multihead.pth` (full fine-tune on Stanford Cars-196, ~2 hr). `upload_weights.py` converts both to `.safetensors` and pushes them + class JSONs to HF Hub. See §4.6.1.
- **Phase 2 (Labeler tab) is DEFERRED** as of 2026-05-22. Phase 3's v0 classifier ships predictions trained entirely on public datasets — the Labeler tab from the original spec (`docs/superpowers/specs/2026-05-21-vehicle-attribute-classification-design.md` §Phase 2) is only revisited if real-world transfer quality on the user's cameras turns out to be unusable.
