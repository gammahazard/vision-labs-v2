...
25. **The detector "registry gate" exits cleanly** — pose/vehicle/face-recognizer all run `_check_camera_wants_detector()` at startup; clean exit when the flag is false. Compose uses `restart: on-failure` so the exit stays exited. Toggling `detect_*` in the UI **does** now auto-recreate the affected service: `cameras.py:upsert_camera` detects detector-flag changes vs the existing entry and publishes to `config:apply` Redis channel with per-cam service names (`pose-detector-camN`, `vehicle-detector-camN`, `face-recognizer-camN`); the orchestrator's per-cam expansion logic from PR #10 fires `docker compose up -d --force-recreate` on the affected service so it re-reads the flag at startup. Before this fix the orchestrator's `reconcile()` only watched `enabled` and toggling detector flags mid-life was a no-op.
