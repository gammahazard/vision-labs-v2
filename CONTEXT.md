...
- **Training pipeline** at `scripts/vehicle_attributes/`:
  ...
  - **Per-operator retrain (Phase 4)** — `collect_labels.py` walks the per-track metadata.json files for user_labels blocks; `finetune_heads.py` fine-tunes one head (color or body) against those labels with warm-start + class-weighted CE + 20% held-out val; `retrain_attributes.py` is the orchestrator CLI that prompts, calls finetune, refuses deploy on val regression, and writes a deploy recipe. Invoked via `docker compose exec vehicle-attributes-cam1 python /workspace/scripts/vehicle_attributes/retrain_attributes.py --heads color,body --classes-dir /app/classes` — note the `python` interpreter (the container's `python3` is the system 3.10 without cv2/torch/timm). Body retrain re-emits a full `multihead_<version>.safetensors` with body_head replaced and backbone/make_head/model_head bit-identical (see `_save_merged_multihead`).
- **Phase 4 labeling (Phase 2 of the original spec, REVISITED 2026-05-23).** The user-facing labeling tools shipped:
  - **Per-track labeling backend** (PR #78): `POST /api/browse/label/{date}/{camera}/{track_dir}` writes a `user_labels` block to metadata.json atomically; `GET /api/browse/label-classes` returns the 4 class lists for dropdowns.
  - **Inline cropsModal labeling UI** (PR #79): per-track Label/Edit/Skip form with color/body dropdowns + make/model autocomplete; IR-mode tracks disable the color dropdown.
  - **Crop-pruning curation tools** (PR #81): per-thumb ✕ removes one bad crop (auto-promotes the next angle to hero, refuses last-crop with 409 to preserve labels); per-track "Delete track" nukes the whole `vehicle_*/` folder for unsalvageable tracks.
  - The deferred "Labeler tab" from the original spec was the dedicated full-page workflow; it's been replaced by the modal-inline approach above, which co-locates labeling with the existing review surface. Original spec at `docs/superpowers/specs/2026-05-21-vehicle-attribute-classification-design.md` §Phase 2.

### 4.6.1 Vehicle detection → bbox → HD crop, end-to-end

Line-numbered trace of the path from RTSP frame to per-track HD crop on disk. The HD-frame pairing (PR #24) is the load-bearing piece — without it the crop landed on background instead of the car for fast-moving traffic.

1. **camera-ingester reads HD frame + writes Redis** — `services/camera-ingester/ingester.py:544`. `r_hd.setex(HD_FRAME_KEY, 5, jpeg_buf.tobytes())` — HD JPEG bytes with 5 s TTL on `frame_hd:cam{N}`. Separate thread from the sub-stream loop; ~5 fps. Without this key being fresh, every downstream consumer fails-soft.
2. **vehicle-detector reads sub-stream + runs YOLO** — `services/vehicle-detector/detector.py:258` decodes the sub-stream frame; YOLO inference produces `detections` (list of `{bbox, class_name, confidence}`) in sub-stream coords (896×512).
3. **vehicle-detector pairs HD bytes inline** — `detector.py:347-349`. `hd_bytes = r.get(HD_FRAME_KEY); if hd_bytes: det_msg["hd_frame_bytes"] = hd_bytes`. Fetched at emit time, NOT at next consumer's read time, so we capture the HD frame whose timestamp is closest to the bbox's compute moment. Field absent if HD TTL just expired (5 s) — downstream falls back to its own lookup.
4. **vehicle-detector publishes to detection stream** — `detector.py:351`. `r.xadd(DETECTION_STREAM, ...)` where `DETECTION_STREAM = detections:vehicle:cam{N}`. Stream `maxlen=1000` so ~100 s of history at 5 fps; OK for live consumers, useless for forensic queries.
...
12. **Idle check + sample emit** — `manager.py:317-328`. If duration ≥ `VEHICLE_IDLE_TIMEOUT` + stationary + not already alerted → `_emit_vehicle_idle_event`. Then every `SAMPLE_INTERVAL_FRAMES`-th matched update (default 3) → `_emit_vehicle_sample_event`.
13. **New track: create + stash HD + first snapshot** — `manager.py:332-358`. Mint `vehicle_{N:04d}`, build `TrackedVehicle`, stash `hd_frame_bytes`, save sub-stream snapshot to `vehicle_snapshot:cam{N}:{ms}` (24 h TTL), emit `vehicle_detected`.
14. **Sample event writes per-sample HD key** — `manager.py:495-500`. `hd_snapshot_key = f"vehicle_hd_sample:{CAMERA_ID}:{veh.vehicle_id}:{ts_ms}"` then `self.r.setex(hd_snapshot_key, 60, veh.last_hd_frame_bytes)`. 60 s TTL is enough for vehicle-attributes to react.
...
**Why this trace matters:** before PR #24, step 17 was vehicle-attributes' own independent fetch of `frame_hd:cam{N}` at flush time. For fast-moving traffic the HD frame had aged 200-500 ms between when the bbox was computed and when the crop happened — long enough for the car to have moved out of the bbox region, producing crops of empty road. The per-sample HD key in step 14 closes the loop: the bytes and the bbox are now from the same frame.

### 4.6.2 Why two models, not one shared backbone

Phase 3 v0 originally specced a single ConvNeXt-Tiny with 4 heads (color + body + make + model). In practice that doesn't train cleanly:

- The **color** head trains best on a frozen ImageNet backbone — color is a low-level feature the generic backbone already captures, training is fast (~20 min for the head alone), and there's no overfit risk on a 7,690-param head trained against 37,746 VeRi-776 images.
- The **make / model / body** heads need backbone fine-tuning. Fine-grained car-model discrimination requires features the generic ImageNet backbone doesn't expose; a frozen backbone caps out around ~75 % accuracy on Stanford Cars-196.

These two training regimes can't share the same backbone in one pass: fine-tuning the backbone on Stanford Cars would specialize features away from the generic color-discriminating ones the color head was trained against, and the color head's mapping would silently degrade.

The v0 design is therefore: two models, two backbones, two safetensors files on HF Hub (`color_head_v0.safetensors` carries only the linear head weights — ImageNet backbone is recreated from timm; `multihead_v0.safetensors` carries the fine-tuned backbone + 3 heads). VRAM cost is ~1.2 GB total instead of ~600 MB, still well within the small-tier budget. Both load lazily on first classify call.
...
- **`browse.py`** (`/api/browse`) — `GET /days`, `GET /days/{date}`, `GET /snapshot/{cam_or_legacy}/{date}/{file}`, legacy `GET /snapshot/{date}/{file}`, `GET /faces`, `GET /tracks/{date}` (Phase 1 per-track HD-crop groups), `GET /label-classes` (color/body/make/model class lists for the labeling UI), `POST /label/{date}/{camera}/{track_dir}` (atomic user_labels write into metadata.json; Phase 4 PR1), `DELETE /tracks/{date}/{camera}/{track_id}/{filename}` (remove one crop, promote next angle to hero, refuse last-crop with 409; PR #81), `DELETE /tracks/{date}/{camera}/{track_id}` (nuke whole track folder; PR #81).
...
Slot block ranges in docker-compose.yml drift quickly — the file is ~4400 lines after Phase 3 added a 7th per-cam service (`vehicle-attributes-camN`) on 2026-05-22. Run `grep -n "^  <service>-camN:" docker-compose.yml` for any per-cam prefix to find the exact line for a slot. **Future work:** dynamic slot generation via orchestrator-written `docker-compose.override.yml` would remove the 20-slot ceiling entirely.
...
### Tests
- ~440 tests, 0 quarantined as of 2026-05-22. Run via `source .venv-test/bin/activate && pytest -q`.
