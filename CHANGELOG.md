...
### Fixed
- **YOLO car↔truck↔bus flicker was splitting one vehicle into multiple tracks** — `_try_ghost_match` + `_try_live_center_match` required strict class equality; YOLOv8 frequently flips between car/truck/bus on the same physical vehicle (pickup at 11:35 became vehicle_0009 'car' + vehicle_0010 'truck' 2 s later). Now treats 4-wheel classes as interchangeable in the fallback paths; bicycle/motorcycle stay strict. Added a same-frame guard so two distinct vehicles in one detection batch can't merge via the looser threshold. *Requires tracker rebuild.*
