...
- **`vehicle_left` spamming the events panel for drive-by cars** — every ghost-buffer expiry was firing `vehicle_left` regardless of whether the car had ever gone idle. For drive-bys this produced noise (a "left" event for a car that just appeared once and disappeared); when the IoU tracker briefly split one physical car into two TrackedVehicles, the noise doubled. Split the producer: a new internal `vehicle_gone` event fires at every ghost-buffer expiry (carries `was_idle` boolean for downstream classification) and is the flush trigger for the vehicle-attributes service; `vehicle_left` now fires only when `idle_alerted=True` was set during the track's life, making it the strict "your parked car drove off" event. *Requires tracker rebuild + vehicle-attributes rebuild.*
- **IoU identity-swap on fast-moving vehicles** — same physical car drifting across consecutive frames could have IoU < threshold (live cam1 data: 50px center shift, 75px-wide bbox → IoU≈0.14, below the 0.2 threshold) and spawn a second TrackedVehicle. The duplicate track produced two `vehicle_detected` events plus two ghost-expiry events. Added a center-distance fallback after the IoU match step (`_try_live_center_match`), mirroring `_try_ghost_match`'s heuristic — same-class only, threshold `bbox_w × VEHICLE_GHOST_MAX_DIST_RATIO (=2.0)`. The IoU step stays class-blind (handles YOLO class flicker on the same vehicle); the looser center-distance path is class-aware to avoid cross-class merges. *Requires tracker rebuild.*
