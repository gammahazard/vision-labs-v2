...
### Fixed
- **Parked-car crops captured the passing vehicle that occluded them** — when a moving vehicle drove through (or sat in front of) a parked car's bbox, the HD-frame crop included pixels of both vehicles, polluting the classifier's color/body/make/model vote. `vehicle_sample` now skips emission when another currently-tracked non-idle vehicle's bbox overlaps this track by IoU > `SAMPLE_OCCLUSION_IOU_THRESHOLD` (default 0.15). Idle-vs-idle overlaps (two adjacent parked cars) are exempt. Reservoir still fills from unoccluded frames before/after the drive-by. *Requires tracker rebuild.*
- **IoM rescue lost to non-idle competitors for the parked car's detections** — the post-loop IoM rescue (added in this release) only ran when no IoU match existed; a fresh non-idle track that briefly overlapped at IoU 0.40 stole the idle car's bbox-jittered detections. Reordered: IoM rescue now runs BEFORE the primary IoU loop, so idle tracks are sticky. Observed live at 12:44–12:46 on cam1: a passing truck stole the parked Honda's detections, eventually spawning a phantom vehicle_0003 that fired a duplicate vehicle_idle 150 s later. *Requires tracker rebuild.*
- **Duplicate `vehicle_idle` alerts when YOLO bbox-jittered the parked car** — tight 0.65 idle-IoU gate rejected a jittered detection whose IoM was 0.99 (same parked Honda, bbox 35 px wider); the rejection spawned a phantom track that fired its own idle alert 150 s later. New `_try_idle_iom_match` rescue: if IoM ≥ `VEHICLE_IDLE_IOM_THRESHOLD` (0.9) AND area ratio ≤ `VEHICLE_IDLE_IOM_AREA_RATIO_MAX` (2.0), absorb into the idle track. Area-ratio gate prevents person-inside-truck false merges. *Requires tracker rebuild.*
