...
- ~~**Phase D.5**: ONVIF network camera discovery~~ — **DROPPED.** Gate failed: WSL2 host-networking containers can't reliably do multicast (both WS-Discovery and SSDP returned 0 responders during May-2026 test). Manual RTSP entry stays the only camera-adding flow in v1. See PACKAGING_PLAN.md for the empirical results.
